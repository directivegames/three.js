...
class GTAONode extends TempNode {
	...
	setup( builder ) {
		...
		this._ao = Fn( () => {
			...
			const probeClip = this._cameraProjectionMatrix.mul( vec4( viewPosition.add( vec3( this.radius, 0.0, 0.0 ) ), 1.0 ) );
			const screenRadiusPx = abs( getScreenPositionFromClip( probeClip ).x.sub( getScreenPositionFromClip( clipPosition ).x ) ).mul( this._resolution.x );
			const radiusScale = min( float( 1.0 ), this.maxScreenRadius.div( max( screenRadiusPx, float( 1e-4 ) ) ) );
			const radius = this.radius.mul( radiusScale );
			// !WITH_GENESYS
